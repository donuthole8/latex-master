\chapter{実験}
  ...
  \section{実験結果}
    ...
    \subsection*{土砂領域マスク画像作成}
      ...
      \ref{災害後オルソ画像の前処理}項のオルソ画像の領域分割結果を入力とし，土砂領域マスク画像作成を行った．実験1-3におけるヒストグラム均一化，土砂候補領域検出，植生領域検出，，土砂領域検出，土砂領域マスク画像作成の各処理結果を\fref{土砂領域マスク画像作成結果（実験1）}から\fref{土砂領域マスク画像作成結果（実験3）}に示す．なお，赤色の領域が土砂候補領域及び土砂領域，緑色の領域が植生領域である．
  ...
  \section{精度評価}
    ...
    \begin{table}[b]
      ...
      \begin{tabular}{cccc}
        \hline
        \textbf{メッシュ番号（x,y）} & 結果角度[$^{\circ}$] & 正解角度[$^{\circ}$] & \textbf{精度} \\
        \hline  \hline
        1  (0,0)  & 270 & 170 & 0.443 \\
        6  (2,0)  & 280 & 160 & 0.333 \\
  ...
  \section{考察}
    \ref{実験結果}節の土砂移動推定の精度について，本手法の問題と有用性について考察を行った．
    ...
    \subsection{土砂移動推定の精度について}
      ...
      \subsubsection*{災害後DSMの正規化}
      \subsubsection*{土砂領域マスク画像作成における精度}
      \subsubsection*{土砂量推定における精度}
      \subsubsection*{土砂移動推定における精度}
